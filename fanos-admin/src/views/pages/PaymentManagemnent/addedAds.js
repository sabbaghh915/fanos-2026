...
const AddedAds = (props) => {
  ...
  const AllTransactionHistory = async () => {
    ...
    try {
      const res = await Axios({
        method: "GET",
        url: ApiConfig.paymentAddedAds,
        headers: {
          token: window.localStorage.getItem("token"),
        },
        params: {
          page: page,
          orderType:"AddedProduct",
  ...
  const downloadExcel = () => {
    const fileName = `Added_ads_Payment(${moment(new Date()).format('MMMM Do YYYY, h:mm:ss a')}).xlsx`;
        ...
        <Box style={{ marginTop: "20px" }} mb={3}>
          <Grid spacing={2} style={{ display: "flex", flexDirection: "row" }}>
            <Grid item lg={12} md={6}>
              <Typography variant="h4">Added Ads</Typography>
            </Grid>
