...
const PopularProduct = function () {
  const classes = useStyles();
  const history = useHistory();
  const [data, setData] = useState([]);
  const [popularProdLimit, setPopularProdLimit] = useState(10);
  const [populProdTotal, setPopuProdTotal] = useState(10);
  ...
  const getPopularProduct = async () => {
    try {
      const res = await Axios({
        ...
        params: {
          limit: popularProdLimit,
        },
      });

      if (res.data.responseCode === 200) {
        setData(res?.data?.result?.data);
        setPopularProdLimit(res?.data?.result?.limit);
        setPopuProdTotal(res?.data?.result?.totalItems);
  ...
  return (
    <Box className={classes.mainBox}>
      <MainCarousel />
      {data?.length === 0 ? null : populProdTotal > popularProdLimit ? (
        pages > 1 ? (
          <>
                      ...
                      <Box className={classes.productDiv}>
                        ...
                        <Box className={classes.bottomFlex}>
                          ...
                          <Box className={classes.priceContainer}>
                            ...
                            <p className={classes.price}>{values.price}{values.currency}</p>
                          </Box>
                        </Box>
                      </Box>
                    </Box>
      ...
      <Box className={classes.loadMoreBtnContainer}>
        <Button
          ...
          onClick={() => {
            setPopularProdLimit(popularProdLimit + 10);
          }}
