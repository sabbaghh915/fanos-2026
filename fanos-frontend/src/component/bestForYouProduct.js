...
const BestForYouProduct = function (props) {
  const classes = useStyles();
  const [data, setData] = useState([]);
  const [dataAds, setDataAds] = useState([]);
  const history = useHistory();

  const getbestForYouProduct = async () => {
  try {
    const userId = localStorage.getItem("userId"); // اجلب userId من التخزين المحلي
    const res = await Axios({
      method: "GET",
      url: `${ApiConfig.getbestForYouProduct}?userId=${userId}`,
    });
    if (res.data.responseCode === 200) {
      const result = res?.data?.result?.docs || res?.data?.result;
      // تأكد من أن البيانات مصفوفة - إذا كانت كائن واحد، ضعها في مصفوفة
      if (Array.isArray(result)) {
        setData(result);
      } else if (result && typeof result === 'object') {
        setData([result]); // إذا كانت كائن واحد، ضعها في مصفوفة
      } else {
        setData([]); // إذا كانت null أو undefined، استخدم مصفوفة فارغة
      }
    } else {
      toast.warn("Sign in First");
      setData([]); // تأكد من أن البيانات مصفوفة فارغة في حالة الخطأ
    }
  } catch (error) {
  ...
  const sliceData = Array.isArray(data) && data.length >= 5 ? data.slice(0, 5) : (Array.isArray(data) ? data : []);

  useEffect(() => {
    getbestForYouProduct();
    getAllAdvertisement();
  }, []);
  ...
  return (
    <Box className={classes.mainBox}>
      <Box className={classes.flexBox}>
        <Box>
          <Typography className={classes.heading}>Best For You Products</Typography>
        </Box>
                  ...
                  >
                    <div className={classes.imageContainer}>
                      <img
                        ...
                        src={data.productImage && Array.isArray(data.productImage) && data.productImage.length > 0 
                          ? data.productImage[0] 
                          : "/images/logo.svg"}
                      />
                    </div>
...
export default BestForYouProduct;
