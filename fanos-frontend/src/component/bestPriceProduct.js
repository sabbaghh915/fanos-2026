...
const BestPriceProduct = function (props) {
  ...
  try {
    const res = await Axios({
      method: "GET",
      url: ApiConfig.getbestPriceProduct,
    });

    if (res.data.status === true && res.data.data) {
      // تأكد من أن البيانات مصفوفة
      const productData = res.data.data;
      setData(Array.isArray(productData) ? productData : [productData]);
      console.log("Best price product frontend:", productData);
    } else {
      toast.warn("No product found");
      setData([]); // تأكد من أن البيانات مصفوفة فارغة في حالة عدم وجود منتجات
    }
  } catch (error) {
    toast.error("Error while fetching best price product");
    console.error(error);
  ...
  return (
    <Box className={classes.mainBox}>
      <Box className={classes.flexBox}>
        <Box>
          <Typography className={classes.heading}>Best Price Products</Typography>
        </Box>
...
export default BestPriceProduct;
