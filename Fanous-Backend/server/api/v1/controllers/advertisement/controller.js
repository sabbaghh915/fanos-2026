...
export class advertisementController {
  ...
  async addadvertisement(req, res, next) {
    const validationSchema = Joi.object({
      ...
      advertisementType: Joi.string().valid("TYPE_1", "TYPE_2").required(),
    });
    try {
      ...
      // Ensure advertisementType is set
      if (!validatedBody.advertisementType) {
        validatedBody.advertisementType = "TYPE_1"; // Default value
      }
  ...
  async updateAdvertisementData(req, res, next) {
    const validationSchema = Joi.object({
      ...
      img: Joi.string().optional(),
      advertisementType: Joi.string().valid("TYPE_1", "TYPE_2").optional(),
    });
    try {
  ...
  async getAllAdvertisement(req, res, next) {
    try {
      ...
      // Support filtering by advertisementType if provided
      if (req.query.advertisementType) {
        query.advertisementType = req.query.advertisementType;
      }
      var result = await advertisementFind(query);
