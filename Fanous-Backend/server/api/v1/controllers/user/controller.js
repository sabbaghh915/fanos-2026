...
export class userController {
  ...
  async signup(req, res, next) {
    ...
    try {
      ...
      validatedBody.password = hashSync(validatedBody.password, 10);
      validatedBody.otpVerification = true; // تعطيل التحقق من OTP
      // validatedBody.otp = commonFunction.getOTP();
      // validatedBody.otpTime = new Date().getTime() + 180000;

      if (userInfo && userInfo.otpVerification == false) {
        // await commonFunction.sendEmailOtpForSignup(email, validatedBody.otp);
        const updateUserData = await updateUser({ _id: userInfo._id }, validatedBody);
        const obj = {
          _id: updateUserData._id,
          email: updateUserData.email,
          // otp: updateUserData.otp,
        };
        return res.json(new response(obj, responseMessage.USER_SIGNUP_SUCCESS));
      }
  ...
  async getPopularProducts(req, res, next) {
    try {
      const allProductData = await wishlist({ status: { $ne: status.DELETE } });
      
      // If no wishlist data, return empty list instead of 404
      ...
      const result = Object.entries(
        allProductData.reduce((acc, obj) => {
          const productIdStr = obj.productId ? obj.productId.toString() : null;
          if (productIdStr && productIdStr in acc) {
            acc[productIdStr].count++;
          } else if (productIdStr) {
            acc[productIdStr] = { count: 1 };
          }
          return acc;
        }, {})
      )
      ...
      const responseData = allPopularProducts
        .map((item) => {
  ...
  async getBestPriceProduct(req, res, next) {
    try {
      const allProducts = await productServices.findAllProduct({});

      if (!allProducts || allProducts.length === 0) {
        return res.status(404).json({ status: false, message: "لا توجد منتجات", data: null });
      }
  ...
  async getbestForYouProduct(req, res, next) {
    ...
    try {
      const validatedBody = await validationSchema.validateAsync(req.query);
      let { userId, page = 1, limit = 10 } = validatedBody;

      // Fix: Check if userId is valid (not "undefined" string or empty)
      if (userId === "undefined" || userId === "" || !userId) {
        userId = null;
      }

      // Validate ObjectId format if userId is provided
      ...
      if (userId) {
        const recentSearches = await SearchModel.find({ userId: new mongoose.Types.ObjectId(userId) })
          .sort({ createdAt: -1 })
          .limit(5);

      searchKeywords = recentSearches
        .map(entry => entry.serchText) // Fixed: use serchText (with typo) as per model
        .filter(Boolean);
    }
    ...
    if (searchKeywords.length > 0) {
      ...
      addFieldsStage = {
        $addFields: {
          relevance: {
            $cond: [
              { $regexMatch: { input: "$productName", regex: searchKeywords[0], options: "i" } },
              2,
              {
                $cond: [
                  { $regexMatch: { input: "$description", regex: searchKeywords[0], options: "i" } },
                  1,
                  0,
                ],
              }
    ...
    const aggregationPipeline = [
      matchStage,
      ...(searchKeywords.length > 0 ? [addFieldsStage] : []),
    ...
    // Return empty list instead of 404 if no products found
    ...
    const totalCountResult = await Product.aggregate([...countPipeline, { $count: "total" }]);
